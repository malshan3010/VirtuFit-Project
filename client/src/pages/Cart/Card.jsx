...
//               </table>
//               <button
//                 className="mt-4 w-10 h-10 bg-pink-500 rounded-full shadow-lg hover:bg-purple-600 transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 duration-300 flex justify-center items-center"
//                 onClick={downloadPDF}
//               >
...
//               <button
//                 type="submit"
//                 className="w-full mt-4 px-4 py-2 bg-gradient-to-r from-purple-500 via-pink-500 to-red-500 text-white font-bold rounded-md shadow-lg transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-100 duration-300"
//               >
...
const CardDetails = () => {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'cardNumber') {
      ...
      setCardInfo({ ...cardInfo, [name]: formattedValue.trim() });
    } else if (name === 'expiryDate') {
      let formattedValue = value.replace(/\D/g, '');
      if (formattedValue.length > 2) {
        formattedValue = `${formattedValue.slice(0, 2)}/${formattedValue.slice(2, 4)}`;
    ...
    try {
      ...
      if (!cardExists) {
        const saveCardResponse = await fetch('http://localhost:3000/card/save', {
          ...
          body: JSON.stringify({ ...cardInfo, cardNumber: cardInfo.cardNumber.replace(/\s/g, '') }),
        });

        if (!saveCardResponse.ok) {
          Swal.fire({ title: 'Card Save Failed', icon: 'error', text: 'Unable to save card.' });
          return;
        }

        const savedCardData = await saveCardResponse.json();
        setSavedCards([...savedCards, savedCardData.card]);
      }

      const paymentResponse = await fetch('http://localhost:3000/card/process-payment', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...cardInfo, cardNumber: cardInfo.cardNumber.replace(/\s/g, ''), totalAmount }),
      });
      ...
      if (paymentResponse.ok) {
        ...
        Swal.fire({
          title: 'Order Successful!',
          text: 'Your order has been placed and cart cleared.',
          icon: 'success',
        }).then(() => navigate('/'));
      } else {
      ...
      Swal.fire({ title: 'Error', text: 'Unexpected error occurred.', icon: 'error' });
    }
  };
  ...
  const handleSavedCardClick = async (card) => {
    try {
      const paymentResponse = await fetch('http://localhost:3000/card/process-payment', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...card, totalAmount }),
      });

      if (paymentResponse.ok) {
        await fetch('http://localhost:3000/cart/', { method: 'DELETE' });
        Swal.fire({ title: 'Order Successful!', icon: 'success' }).then(() => navigate('/'));
      } else {
        Swal.fire({ title: 'Payment Failed', icon: 'error' });
      }
    } catch (error) {
      Swal.fire({ title: 'Error', text: 'Unexpected error.', icon: 'error' });
    }
  };
  ...
  const downloadPDF = () => {
    ...
    const gradientColors = [
      { color: [34, 197, 94], position: 10 },
      { color: [13, 148, 136], position: 25 },
      { color: [6, 182, 212], position: 40 },
    ];
    gradientColors.forEach(({ color, position }) => {
      doc.setFillColor(...color);
      doc.rect(position, 10, 30, 10, 'F');
    });
    doc.text('VirtuFit.LK', 15, 18);

    doc.setFontSize(18);
    doc.setTextColor(0, 0, 0);
    doc.text('Order Summary', 20, 40);
    ...
    doc.setFontSize(16);
    doc.text(`Total Amount Due: Rs ${totalAmount.toFixed(2)}`, 20, pageHeight - 20);
      ...
      <div className="shadow-lg rounded-lg p-6 bg-white">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <div className="space-y-2">
              {savedCards.map((card, index) => (
                <button key={index} onClick={() => handleSavedCardClick(card)} className="w-full px-4 py-2 bg-gray-100 border rounded-lg shadow-sm text-left">
                  {card.cardholderName} - **** **** **** {card.cardNumber.slice(-4)}
                </button>
              ))}
            </div>
                ...
                <tbody>
                  {cartItems.map((item, index) => (
                    <tr key={index} className="border-b">
                      ...
                      <td className="py-2 px-4 text-right">Rs {item.ItemPrice.toFixed(2)}</td>
                      <td className="py-2 px-4 text-right">Rs {(item.ItemPrice * item.Quantity).toFixed(2)}</td>
                    </tr>
                  ))}
                  <tr>
                    <td colSpan="3" className="py-2 px-4 text-right font-semibold">Delivery Fee:</td>
                    <td className="py-2 px-4 text-right">Rs {deliveryFee.toFixed(2)}</td>
                  </tr>
                  <tr>
                    <td colSpan="3" className="py-2 px-4 text-right font-bold">Total:</td>
                    <td className="py-2 px-4 text-right font-bold">Rs {totalAmount.toFixed(2)}</td>
                  </tr>
                </tbody>
              </table>
              <button onClick={downloadPDF} className="mt-4 bg-pink-600 hover:bg-purple-700 text-white py-2 px-4 rounded-lg w-full">
          ...
          <div>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input name="cardholderName" value={cardInfo.cardholderName} onChange={handleChange} className="w-full p-2 border rounded" />
                {errors.cardholderName && <p className="text-red-500 text-sm">{errors.cardholderName}</p>}
              </div>
              <div>
                <label className="block font-medium">Card Number</label>
                <input name="cardNumber" value={cardInfo.cardNumber} onChange={handleChange} className="w-full p-2 border rounded" />
                {errors.cardNumber && <p className="text-red-500 text-sm">{errors.cardNumber}</p>}
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block font-medium">Expiry Date (MM/YY)</label>
                  <input name="expiryDate" value={cardInfo.expiryDate} onChange={handleChange} className="w-full p-2 border rounded" />
                  {errors.expiryDate && <p className="text-red-500 text-sm">{errors.expiryDate}</p>}
                </div>
                <div>
                  <label className="block font-medium">CVC</label>
                  <input name="cvc" value={cardInfo.cvc} onChange={handleChange} className="w-full p-2 border rounded" />
                  {errors.cvc && <p className="text-red-500 text-sm">{errors.cvc}</p>}
                </div>
              </div>
              <button type="submit" className="bg-pink-600 hover:bg-purple-700 text-white py-2 px-4 rounded-lg w-full">
                Pay Rs {totalAmount.toFixed(2)}
              </button>
            </form>
          </div>
